...
static inline std::string get_xml_string(napi_env env, napi_value* argv) {
  size_t size;
  napi_get_value_string_utf8(env, argv[0], NULL, 0, &size);
  std::string xml_string(size + 1, '\0');
  napi_get_value_string_utf8(env, argv[0], xml_string.data(), size + 1, NULL);
  xml_string.resize(size);
  return xml_string;
}
...
napi_value xml2json(napi_env env, napi_callback_info info) {
  ...
  const std::string xml_string = get_xml_string(env, argv);

  try {
    napi_value result;
    napi_create_string_utf8(env, xml2json(xml_string.c_str()).c_str(),
